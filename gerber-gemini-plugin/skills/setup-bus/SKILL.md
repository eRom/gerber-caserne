...
`mcp_airtable_ping` doit retourner `pong`. Sinon : `/plugin install airtable` → `/reload-plugins` → `/mcp` pour authentifier.

## Étape 1 — Workspace

```
mcp_airtable_list_workspaces
```
...
```
mcp_airtable_list_tables_for_base({ baseId: "<base_id>" })
```
