# AGENTS.md — gerber-caserne

Les IDs Airtable (bus messages) et Linear (team / workflows) sont dans `~/.Codex/GERBER.md` (contexte global). Section `## Messages bus` retirée de ce fichier 2026-05-18.
...
| 5 | Codex Desktop utilise des `redirect_uri` localhost éphémères (`http://localhost:<port>/callback`). `ALLOWED_REDIRECT_PATTERNS` doit accepter `localhost`, `127.0.0.1`, `Codex.ai`, `Codex.com` | `src/oauth.ts` |
| 6 | `access_token` de `/token` = `STREAM_TOKEN` (statique), pour que Bearer Managed Agents et OAuth Codex.ai partagent la même verif `/mcp/stream`. Si on rotate `STREAM_TOKEN`, propager partout (env shell `GERBER_TOKEN`, Vault Anthropic) | `src/oauth.ts` |
...
- `/gerber:setup-code` — initialise `.Codex/settings.json` + `AGENTS.md` selon la stack technique
...
- `/gerber:onboarding` — initialise un projet (Linear + GitHub + _gerber_/ + vault RAG + AGENTS.md)
