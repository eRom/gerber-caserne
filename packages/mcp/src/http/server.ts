...
export async function startHttpServer(
  ...
  // by the OAuth router's catch-all responses. Needed for Docker HEALTHCHECK
  // + Traefik probes from outside.
  app.get("/health", (_req, res) => {
    try {
      db.prepare('SELECT 1 AS ok').get();
      res.json({ ok: true, dbPath: db.name });
    } catch (err) {
  ...
  // mount the MCP OAuth authorization server. claude.ai's custom connector uses
  // this flow. If publicUrl is missing, OAuth is silently skipped and only
  // Managed Agents (Vault Bearer) can reach /mcp/stream.
