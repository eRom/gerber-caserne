...
export function Home({ onOpenProject }: HomeProps) {
  ...
  const messages = useData(() => listMessages({ status: 'pending', limit: 20 }));
  ...
  useInput((input, key) => {
    ...
    if (focus === 'projects') {
      ...
      if (key.downArrow) setProjIdx((s) => Math.min(projItems.length - 1, s + 1));
      if (key.return) {
        const proj = projItems[projIdx];
        if (proj) onOpenProject({ id: proj.id, slug: proj.slug, name: proj.name, color: proj.color ?? undefined });
      }
    }
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      ...
      <Box>
        ...
        <Box flexDirection="column" width="30%">
          ...
          {projects.loading ? (
            ...
            <Text dimColor italic>  No projects</Text>
          ) : (
            projItems.map((proj: Project, i: number) => (
              <Box key={proj.id}>
                ...
                <Text
                  {...(focus === 'projects' && projIdx === i ? { bold: true } : {})}
                >
                  {proj.name}
                </Text>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Tab switch focus  |  ↑↓ navigate  |  Enter open{focus === 'messages' ? '/mark done' : ''}  |  [r] refresh
        </Text>
      </Box>
    </Box>
  );
